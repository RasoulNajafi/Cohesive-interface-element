# Python code to generate zero thickness cohesive interface elements in 2D
# The bulk elements are S3 (2D three nodes) and cohesive element type is (COH2D4)
# generated by Rasoul Najafi
import pandas as pd
import numpy as np
...
for i in range (0, len(inclusions_elemIndex)):
    ...
    for j in range (0, len(elem_test)):
        elem = elem_test[j]
        elemConn = elemConnList['Element_Connectivity'][elem-1]
        test= [elem, incl_test, elemConn]
...
for i in range(0, len(inclusions_elemTotal)):
    elementx = inclusions_elemTotal[0][i]
    x_inclusion_id = inclusions_elemTotal[1][i]
    xnodes = inclusions_elemTotal[2][i].tolist()
     
    for j in range(1, len(inclusions_elemTotal)):
        
        elementy = inclusions_elemTotal[0][j]
        y_inclusion_id = inclusions_elemTotal[1][j]
        ynodes = inclusions_elemTotal[2][j].tolist()
